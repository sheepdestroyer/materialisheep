...
public class UserServicesClientTest {
    ...
    public void execute_throwsIOException_emitsError() throws Exception {
        ...
        when(mCallFactory.newCall(any(Request.class))).thenReturn(mCall);
        when(mCall.execute()).thenThrow(ioException);

        java.lang.reflect.Method executeMethod = UserServicesClient.class.getDeclaredMethod("execute", Request.class);
        executeMethod.setAccessible(true);

        @SuppressWarnings("unchecked")
        io.reactivex.rxjava3.core.Observable<Response> observable = (io.reactivex.rxjava3.core.Observable<Response>) executeMethod.invoke(mClient, request);
