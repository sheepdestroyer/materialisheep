...
public class FavoriteActivity extends BaseListActivity {
  ...
  private final Observer<Uri> mObserver =
      uri -> {
        ...
        if (FavoriteManager.Companion.isRemoved(uri)) {
          ...
          if (selected != null && TextUtils.equals(selected.getId(), uri.getLastPathSegment())) {
            onItemSelected(null);
          }
   ...
   * client used the {@link Intent#FLAG_ACTIVITY_SINGLE_TOP} flag when calling {@link
   * #startActivity}.
   *
  ...
  protected void onNewIntent(Intent intent) {
    ...
    FavoriteFragment fragment =
        (FavoriteFragment) getSupportFragmentManager().findFragmentByTag(LIST_FRAGMENT_TAG);
    if (fragment != null) {
