...
public class FeedbackActivity extends ThemedActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    AppUtils.setTextWithLinks(
        (TextView) findViewById(R.id.feedback_note),
    ...
    sendButton.setOnClickListener(
        v -> {
          ...
          mFeedbackClient.send(
              title.getText().toString(), body.getText().toString(), new FeedbackCallback(this));
        });
  }
  ...
  void onFeedbackSent(boolean success) {
    Toast.makeText(
            this, success ? R.string.feedback_sent : R.string.feedback_failed, Toast.LENGTH_SHORT)
        .show();
    if (success) {
      finish();
    } else {
