...
public abstract class ItemRecyclerViewAdapter<VH extends ItemRecyclerViewAdapter.ItemViewHolder>
  ...
  private void load(int adapterPosition, Item item) {
    ...
    mItemManager.getItem(
        item.getId(), mCacheMode, new ItemResponseListener(this, adapterPosition, item));
  }
  ...
  private void highlightUserItem(VH holder, Item item) {
    ...
    holder.mContentView.setBackgroundColor(
        highlight ? mCardHighlightColorResId : mCardBackgroundColorResId);
  }

  private void decorateDead(VH holder, Item item) {
    holder.mContentTextView.setTextColor(
        item.isDead() ? mSecondaryTextColorResId : mTertiaryTextColorResId);
  }
  ...
  private void toggleCollapsibleContent(final VH holder, final Item item, int lineCount) {
    ...
    holder.mReadMoreTextView.setOnClickListener(
        v -> {
  ...
  private void bindActions(final VH holder, final Item item) {
    ...
    holder.mMoreButton.setOnClickListener(
        v ->
            mPopupMenu
                .create(mContext, holder.mMoreButton, Gravity.NO_GRAVITY)
                        ...
                        AppUtils.share(
                            ...
                            item.isStoryType()
                                ...
                                : item.getDisplayedText() == null
                                    ? null
                                    : item.getDisplayedText().toString());
                        return true;
                      }
                      return false;
                    })
