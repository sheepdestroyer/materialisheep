...
public class KeyDelegate {
  ...
  /**
   * Attaches this delegate to the given activity lifecycle. Should call {@link #detach(Activity)}
   * accordingly.
   *
  ...
  public void attach(Activity activity) {
    ...
    mEnabled =
        PreferenceManager.getDefaultSharedPreferences(activity).getBoolean(mPreferenceKey, false);
    PreferenceManager.getDefaultSharedPreferences(activity)
        .registerOnSharedPreferenceChangeListener(mPreferenceListener);
  }

  /**
   * Detaches this delegate from the given activity lifecycle. Should already have called {@link
   * #attach(Activity)}.
   *
  ...
  static class RecyclerViewHelper implements Scrollable {
    ...
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({SCROLL_ITEM, SCROLL_PAGE})
    ...
    public boolean scrollToNext() {
      int
          pos =
              mScrollMode == SCROLL_ITEM
                  ? getLinearLayoutManager().findFirstVisibleItemPosition()
                  : getLinearLayoutManager().findLastCompletelyVisibleItemPosition(),
          next = pos != RecyclerView.NO_POSITION ? pos + 1 : RecyclerView.NO_POSITION;
      if (next != RecyclerView.NO_POSITION && next < mRecyclerView.getAdapter().getItemCount()) {
      ...
      switch (mScrollMode) {
        ...
        default:
          int pos = getLinearLayoutManager().findFirstVisibleItemPosition(),
              previous = pos != RecyclerView.NO_POSITION ? pos - 1 : RecyclerView.NO_POSITION;
          if (previous >= 0) {
