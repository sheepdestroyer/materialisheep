...
public class StoryView extends RelativeLayout implements Checkable {
  ...
  public StoryView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    mCommentDrawable =
        DrawableCompat.wrap(
            ContextCompat.getDrawable(context, R.drawable.ic_comment_white_24dp).mutate());
    ...
    mMoreButton.setBackgroundResource(
        AppUtils.getThemedResId(context, androidx.appcompat.R.attr.selectableItemBackground));
    ta.recycle();
    a.recycle();
  }
  ...
  public void setStory(@NonNull WebItem story, int hotThreshold) {
    if (!mIsLocal && story instanceof Item) {
      ...
      mScoreTextView.setCompoundDrawablesWithIntrinsicBounds(
          0, 0, 0, hot ? R.drawable.ic_whatshot_orange500_18dp : 0);
  ...
  public void setPromoted(int change) {
    ...
    spannable.setSpan(
        new SuperscriptSpan(), 0, spannable.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
    spannable.setSpan(
        new RelativeSizeSpan(0.6f), 0, spannable.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
    spannable.setSpan(
        new ForegroundColorSpan(mPromotedColorResId),
        0,
        spannable.length(),
            ...
            new Animation.AnimationListener() {
              ...
              public void onAnimationEnd(Animation animation) {
                new Handler(android.os.Looper.getMainLooper())
                    .postDelayed(mVoteSwitcher::showNext, VOTE_DELAY_MILLIS);
                mScoreTextView.setText(
                    getContext()
                        .getResources()
