  ...
  public IconButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    setBackgroundResource(
        AppUtils.getThemedResId(
            context, androidx.appcompat.R.attr.selectableItemBackgroundBorderless));
    TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.IconButton, 0, 0);
    int colorDisabled =
        ContextCompat.getColor(
            context, AppUtils.getThemedResId(context, android.R.attr.textColorSecondary));
    int colorDefault =
        ContextCompat.getColor(
            context, AppUtils.getThemedResId(context, android.R.attr.textColorPrimary));
