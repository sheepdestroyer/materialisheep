...
/**
 * Injectable alert dialog builder, allowing swapping between {@link
 * androidx.appcompat.app.AlertDialog.Builder} and {@link android.app.AlertDialog.Builder}. This
 * interface provides a consistent way to create alert dialogs, regardless of the underlying
...
public interface AlertDialogBuilder<T extends Dialog> {
   ...
   *
   * <p>When using a pre-Holo theme, if the supplied view is an instance of a {@link ListView} then
   * the light background will be used.
   *
   * <p><strong>Note:</strong> To ensure consistent styling, the custom view should be inflated or
   * constructed using the alert dialog's themed context
   *
   ...
   * selected item via the supplied listener. The list will have a check mark displayed to the right
   * of the text for the checked item. Clicking on an item in the list will not dismiss the dialog.
   * Clicking on a button will dismiss the dialog.
   *
   * @param items the items to be displayed.
   * @param checkedItem specifies which item is checked. If -1 no items are checked.
   * @param listener notified when an item on the list is clicked. The dialog will not be dismissed
   *     when an item is clicked. It will only be dismissed if clicked on a button, if no buttons
  ...
  /**
   * Creates a {@link Dialog} with the arguments supplied to this builder. It does not {@link
   * Dialog#show()} the dialog. This allows the user to do any extra processing before displaying
   * the dialog. Use {@link #show()} if you don't have any other processing to do and want this to
   * be created and displayed.
   */
  T create();

  /**
   * Creates a {@link Dialog} with the arguments supplied to this builder and {@link
