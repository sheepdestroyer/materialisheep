...
public class ItemActivity extends ThemedActivity implements ItemFragment.ItemChangedListener {
  ...
  public static final String EXTRA_OPEN_COMMENTS =
      ItemActivity.class.getName() + ".EXTRA_OPEN_COMMENTS";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mPreferenceObservable.subscribe(this, this::onPreferenceChanged, R.string.pref_navigation);
    ...
    if (mTabLayout.getTabCount() > 0) {
      ...
      mTabLayout
          ...
          .setText(
              getResources()
                  .getQuantityString(
                      R.plurals.comments_count, item.getKidCount(), item.getKidCount()));
    }
  }
      ...
      buttonArticle.setOnClickListener(
          v ->
              AppUtils.openWebUrlExternal(
                  ItemActivity.this, story, story.getUrl(), mCustomTabsDelegate.getSession()));
    }
    if (mFullscreen) {
      setFullscreen();
    }
  ...
  @Synthetic
  void toggleFabs(
      boolean isComments, FloatingActionButton navigationFab, FloatingActionButton genericFab) {
    AppUtils.toggleFab(
        navigationFab, isComments && Preferences.navigationEnabled(navigationFab.getContext()));
