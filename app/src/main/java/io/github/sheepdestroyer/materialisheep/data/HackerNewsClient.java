...
public class HackerNewsClient implements ItemManager, UserManager {
  ...
  public HackerNewsClient(
      RestServiceFactory factory, SessionManager sessionManager, FavoriteManager favoriteManager) {
  ...
  public void getStories(
      @FetchMode String filter, @CacheMode int cacheMode, final ResponseListener<Item[]> listener) {
    if (listener == null) {
      return;
    }
  ...
  @android.annotation.SuppressLint("CheckResult")
  public void getItem(
      final String itemId, @CacheMode int cacheMode, ResponseListener<Item> listener) {
    if (listener == null) {
      return;
    }
    ...
    switch (cacheMode) {
      ...
      case MODE_CACHE:
        itemObservable =
            mRestService.cachedItemRx(itemId).onErrorResumeNext(t -> mRestService.itemRx(itemId));
        break;
    }
    Observable.defer(
            () ->
        ...
        .observeOn(mMainThreadScheduler)
        .subscribe(
            optionalItem -> listener.onResponse(optionalItem.orElse(null)),
  ...
  public void getUser(String username, final ResponseListener<User> listener) {
    ...
    mRestService
        ...
        .observeOn(mMainThreadScheduler)
        .subscribe(
            optionalUser -> listener.onResponse(optionalUser.orElse(null)),
            t -> listener.onError(t.getMessage() != null ? t.getMessage() : ""));
  }

  @NonNull
  private Observable<Item[]> getStoriesObservable(
      @FetchMode String filter, @CacheMode int cacheMode) {
    Observable<int[]> observable;
    switch (filter) {
      case NEW_FETCH_MODE:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkNewStoriesRx()
                : mRestService.newStoriesRx();
        break;
      case SHOW_FETCH_MODE:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkShowStoriesRx()
                : mRestService.showStoriesRx();
        break;
      case ASK_FETCH_MODE:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkAskStoriesRx()
                : mRestService.askStoriesRx();
        break;
      case JOBS_FETCH_MODE:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkJobStoriesRx()
                : mRestService.jobStoriesRx();
        break;
      case BEST_FETCH_MODE:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkBestStoriesRx()
                : mRestService.bestStoriesRx();
        break;
      default:
        observable =
            cacheMode == MODE_NETWORK
                ? mRestService.networkTopStoriesRx()
  ...
  private Call<int[]> getStoriesCall(@FetchMode String filter, @CacheMode int cacheMode) {
    ...
    if (filter == null) {
      ...
      return cacheMode == MODE_NETWORK
          ? mRestService.networkNewStories()
    ...
    switch (filter) {
      ...
      case SHOW_FETCH_MODE:
        call =
            cacheMode == MODE_NETWORK
                ? mRestService.networkShowStories()
                : mRestService.showStories();
        break;
      case ASK_FETCH_MODE:
        call =
            cacheMode == MODE_NETWORK
                ? mRestService.networkAskStories()
                : mRestService.askStories();
        break;
      case JOBS_FETCH_MODE:
        call =
            cacheMode == MODE_NETWORK
                ? mRestService.networkJobStories()
                : mRestService.jobStories();
        break;
      case BEST_FETCH_MODE:
        call =
            cacheMode == MODE_NETWORK
                ? mRestService.networkBestStories()
                : mRestService.bestStories();
        break;
      default:
        call =
            cacheMode == MODE_NETWORK
                ? mRestService.networkTopStories()
